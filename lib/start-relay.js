...
async function startRelay() {
  ...
  // Resolve relay source — bundled with installer package
  const relaySource = config.relaySourcePath || join(homedir(), ".figma-intelligence", "bridge-relay.js");
  // Also check inside the npm package itself as a last resort
  const bundledSource = join(__dirname, "bridge-relay.js");

  const hasSource = existsSync(relaySource) || existsSync(bundledSource);
  const sourceToUse = existsSync(relaySource) ? relaySource : bundledSource;
